...
graph = ggplot(data = m_data, mapping = aes(x = Year, y = Output, color = Province)) +
  geom_point() +
  geom_line(stat="smooth", method="loess", formula = y~ x, se = FALSE, alpha=0.5) + 
...
north_south_plot = ggplot(data = north_south, mapping = aes(x = Year, y = Output_Ratio)) + 
  geom_point() + 
  geom_smooth(method = 'loess', formula = y ~ x, se = FALSE) +
  ggtitle("North vs South Output Ratio")

east_west = ratio(object1, "Xinjiang_Output", "Zhejiang_Output")

east_west_plot = ggplot(data = east_west, mapping = aes(x = Year, y = Output_Ratio)) + 
  geom_point() + 
  geom_smooth(method = 'loess', formula = y ~ x, se = FALSE) + 
  ggtitle("East vs West Output Ratio")
